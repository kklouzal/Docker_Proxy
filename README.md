...
- The **Override semantics** subtab is intentionally limited to Squid's current six HTTP-semantics-altering `refresh_pattern` options (`override-expire`, `override-lastmod`, `reload-into-ims`, `ignore-reload`, `ignore-no-store`, `ignore-private`). Other `refresh_pattern` options such as `store-stale`, `refresh-ims`, and `max-stale` are handled by the refresh-pattern/default freshness controls rather than this override set.

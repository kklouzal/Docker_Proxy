...
def _req(row: Any) -> PolicyRequest:
    return PolicyRequest(_i(row, "id", 0), _s(row, "proxy_id", 1), _s(row, "status", 2), _s(row, "block_type", 3), _s(row, "client_ip", 4), _s(row, "request_url", 5), _s(row, "domain", 6), _s(row, "category", 7), _s(row, "method", 8), _s(row, "squid_error", 9), _s(row, "user_note", 10), _s(row, "admin_note", 11), _i(row, "created_ts", 12), _i(row, "updated_ts", 13), _i(row, "reviewed_ts", 14), _s(row, "reviewer", 15), _oi(row, "exception_id", 16))


def _exc(row: Any) -> PolicyException:
    return PolicyException(_i(row, "id", 0), _s(row, "proxy_id", 1), _s(row, "status", 2), _s(row, "block_type", 3), _s(row, "client_ip", 4), _s(row, "domain", 5), _s(row, "category", 6), _i(row, "created_ts", 7), _i(row, "updated_ts", 8), _s(row, "created_by", 9), _s(row, "admin_note", 10), _i(row, "expires_ts", 11), _i(row, "revoked_ts", 12), _s(row, "revoked_by", 13), _oi(row, "source_request_id", 14))
...
class PolicyRequestStore:
    ...
    def create_request(self, *, proxy_id: str | None = None, block_type: object = "webfilter", client_ip: object = "", request_url: object = "", domain: object = "", category: object = "", method: object = "", squid_error: object = "", user_note: object = "") -> PolicyRequest:
        ...
        if not d:
            raise ValueError("A valid destination domain is required.")
        if not ip:
            raise ValueError("A valid client IP address is required.")
        now = now_ts()
        with self._connect() as c:
            r = c.execute(f"INSERT INTO {self.REQUEST_TABLE}(proxy_id,status,block_type,client_ip,request_url,domain,category,method,squid_error,user_note,admin_note,created_ts,updated_ts,reviewed_ts,reviewer,exception_id) VALUES(%s,'pending',%s,%s,%s,%s,%s,%s,%s,%s,'',%s,%s,0,'',NULL)", (p, normalize_block_type(block_type), ip, url, d, _text(category, 128), _text(method, 16).upper(), _text(squid_error, 64), _text(user_note, 2000, True), now, now))
            row = c.execute(self._rsql("WHERE id=%s"), (int(r.lastrowid or 0),)).fetchone()
        return _req(row)
...
_store: Optional[PolicyRequestStore] = None

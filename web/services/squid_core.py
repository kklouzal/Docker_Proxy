...
def _exclusive_squid_lifecycle_lock():
    ...
    with _SQUID_LIFECYCLE_LOCK:
        depth = int(getattr(_SQUID_LIFECYCLE_LOCK_STATE, "depth", 0) or 0)
        if depth:
            _SQUID_LIFECYCLE_LOCK_STATE.depth = depth + 1
            try:
                yield
            finally:
                _SQUID_LIFECYCLE_LOCK_STATE.depth = depth
            return
        ...
        try:
            ...
            _SQUID_LIFECYCLE_LOCK_STATE.depth = 1
            yield
        finally:
            _SQUID_LIFECYCLE_LOCK_STATE.depth = 0
...
class SquidController:
    ...
    def _persist_good_config_or_raise(self, config_text: str) -> None:
        try:
            self._persist_good_config(config_text)
        except Exception:
            log_exception_throttled(
                logger,
                "squid_core.persist_config",
                interval_seconds=300.0,
                message="Failed to persist last-known-good Squid config",
            )
            raise
    ...
    def restore_last_known_good_config(
        ...
        *,
        ...
        with _exclusive_squid_lifecycle_lock():
            return self._restore_last_known_good_config_locked(
                reason=reason,
                fallback_config=fallback_config,
            )

    def _restore_last_known_good_config_locked(
        self,
        *,
        reason: str = "",
        fallback_config: str = "",
        ...
        old_persisted_config = self._snapshot_runtime_file(persisted_config_path)
        ...
        def restore_pre_rollback_state(failure: str) -> tuple[bool, str]:
            recovery_parts = [failure]
            try:
                ...
                if current:
                    self._atomic_write_file(self.squid_conf_path, current)
                self._restore_runtime_file_snapshot(
                    icap_include_path,
                    old_icap_include,
                )
                ...
                restart_ok, recovery_detail = self._restart_squid_locked()
                if recovery_detail:
                    recovery_parts.append(recovery_detail)
                if not reread_ok or not restart_ok:
                    recovery_parts.append(
                        "Failed to fully restore the pre-rollback Squid runtime.",
                    )
            except Exception as exc:
                recovery_parts.append(
        ...
        try:
            ...
            return restore_pre_rollback_state(
                public_error_message(
                    exc,
                    default="Failed to persist restored last-known-good Squid config.",
                ),
            )
...
"""
    ...
    def _icap_readiness_status_file(self) -> str:
        return (
            os.environ.get("SQUID_ICAP_READY_STATUS_FILE")
            or "/var/lib/squid-flask-proxy/icap-readiness.json"
        ).strip() or "/var/lib/squid-flask-proxy/icap-readiness.json"

    def _icap_readiness_command(self, command: str = "check") -> list[str]:
        helper = (
            os.environ.get("SQUID_ICAP_READY_HELPER")
            or "/usr/local/bin/icap_readiness.py"
        ).strip() or "/usr/local/bin/icap_readiness.py"
        config_path = str(self._icap_include_path())
        return [helper, command, "--config", config_path]

    def _check_icap_readiness(self, *, timeout: float = 8.0) -> tuple[bool, str]:
        try:
            ...
            return False, public_error_message(
                exc,
                default="ICAP readiness check failed.",
            )

    def _wait_for_icap_readiness(self, *, timeout: float = 75.0) -> tuple[bool, str]:
        try:
            ...
            proc = self._run(
                [
                    *command,
                    "--timeout",
                    str(max(1.0, float(timeout or 75.0))),
                    "--status-file",
                    self._icap_readiness_status_file(),
                ],
                capture_output=True,
                timeout=max(2.0, float(timeout or 75.0) + 5.0),
            )
    ...
    def _accept_running_squid_restart(
        ...
        *,
        ...
        if (supervisor_running or live_pid_running) and listener_ready:
    ...
    def _reconfigure_squid_locked(
        ...
        *,
        ...
        if not ready_ok:
            ok_restart, restart_details = self._restart_squid_locked()
            recovery = (
                "Squid/ICAP readiness was unavailable after reconfigure; "
                + (ready_detail or restart_details or "Squid restart failed.")
            ).strip()
            if ok_restart:
                detail = (detail + "\n" + recovery).strip()
            else:
        ...
        try:
            ...
            return False, "\n".join(
                ...
                for part in [
                    *detail_parts,
                    ready_detail
                    or "Squid process started but the HTTP listener is not responding.",
                ]
                if part
            ).strip()
        except FileNotFoundError:
        ...
        try:
            ...
            if "already running" in direct_start_detail_lower:
                ...
                if ready_ok:
                    detail_parts.append(
                        ...
                        + (" ICAP readiness is green." if ready_detail else ""),
                    )
                    return True, "\n".join(
                        part for part in detail_parts if part
                    ).strip()
            ...
            if proc.returncode == 0 and direct_ready_ok:
                if direct_ready_detail:
                    detail_parts.append(direct_ready_detail)
                return True, "\n".join(part for part in detail_parts if part).strip()
            return False, "\n".join(
                [
                    *detail_parts,

...
class SquidController:
    ...
    def _supervisor_reread_update(self) -> Tuple[bool, str]:
        try:
            reread = run(["supervisorctl", "-c", "/etc/supervisord.conf", "reread"], capture_output=True, timeout=12)
            if reread.returncode != 0:
                return False, self._decode_completed(reread) or "supervisorctl reread failed"
            update = run(["supervisorctl", "-c", "/etc/supervisord.conf", "update"], capture_output=True, timeout=20)
    ...
    def validate_config_text(self, config_text: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            proc = run(["squid", "-k", "parse", "-f", tmp_path], capture_output=True, text=True, timeout=15)
    ...
    def restart_squid(self) -> Tuple[bool, str]:
        try:
            proc = run(["supervisorctl", "-c", "/etc/supervisord.conf", "restart", "squid"], capture_output=True, timeout=12)
        ...
        try:
            shutdown = run(["squid", "-k", "shutdown"], capture_output=True, timeout=8)
    ...
    def clear_disk_cache(self) -> Tuple[bool, str]:
        ...
        try:
            stop = run(["supervisorctl", "-c", "/etc/supervisord.conf", "stop", "squid"], capture_output=True, timeout=20)
            ...
            try:
                ...
                log_exception_throttled(
                    logger,
                    "squid_runtime.shutdown_fallback",
                    interval_seconds=300.0,
        ...
        try:
            prepare = run(["squid", "-z", "-f", self.squid_conf_path], capture_output=True, timeout=90)
    ...
    def apply_config_text(self, config_text: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if workers_changed:
                ...
                if not ok_scale:
                    ...
                    try:
                        ...
                        log_exception_throttled(
                            logger,
                            "squid_runtime.revert_icap_include",
                            interval_seconds=300.0,
                    ...
                    try:
                        ...
                        log_exception_throttled(
                            logger,
                            "squid_runtime.revert_icap_supervisor",
                            interval_seconds=300.0,
                ...
                if not ok_restart:
                    if os.path.exists(backup_path):
                        ...
                        try:
                            ...
                            log_exception_throttled(
                                logger,
                                "squid_runtime.revert_icap_include.restart",
                                interval_seconds=300.0,
                        ...
                        try:
                            ...
                            log_exception_throttled(
                                logger,
                                "squid_runtime.revert_icap_supervisor.restart",
                                interval_seconds=300.0,
                ...
                try:
                    ...
                    log_exception_throttled(
                        logger,
                        "squid_runtime.persist_config.workers",
                        interval_seconds=300.0,
            ...
            reconfigure = run(["squid", "-k", "reconfigure"], capture_output=True, timeout=15)
    ...
    def reload_squid(self):
        try:
            proc = run(["squid", "-k", "reconfigure"], capture_output=True, timeout=15)
    ...
    def get_status(self):
        try:
            proc = run(["squid", "-k", "check"], capture_output=True, timeout=15)

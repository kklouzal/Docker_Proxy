...
_MIME_TOKEN_RE = re.compile(r'^[A-Za-z0-9][A-Za-z0-9.+_-]*/[A-Za-z0-9][A-Za-z0-9.+_-]*$')
...
CLAMAV_FIELDS: tuple[ClamavFieldSpec, ...] = (
    ...
    _field('file_security_preset', 'preset', 'Policy preset', 'Squid + ICAP file policy', 'select', help_text='Balanced is the default foundation: scan uploads and downloads, block common executable payload names, and keep size/archive limits explicit.', choices=(_choice('balanced', 'Balanced'), _choice('monitor', 'Monitor'), _choice('strict', 'Strict'))),
    _field('file_security_scan_downloads', 'coverage', 'Scan downloads', 'adaptation_access av_resp_set', 'checkbox', help_text='Send eligible GET responses through ClamAV RESPMod scanning.'),
    _field('file_security_scan_uploads', 'coverage', 'Scan uploads', 'adaptation_access av_req_set', 'checkbox', help_text='Send POST, PUT, and PATCH request bodies through ClamAV REQMod scanning where Squid/c-icap can adapt the request.'),
    ...
    _field('file_security_block_risky_extensions', 'blocking', 'Block risky extensions', 'http_access deny urlpath_regex', 'checkbox', help_text='Block common script, installer, and binary file names before transfer.'),
    _field('file_security_risky_extensions', 'blocking', 'Risky extensions', 'urlpath_regex', 'text', help_text='Space or comma separated extensions without leading dots.', placeholder='exe dll msi bat cmd com scr ps1 vbs js jar apk'),
    _field('file_security_block_executable_content', 'blocking', 'Block executable payload names', 'http_access deny urlpath_regex / req_header Content-Type', 'checkbox', help_text='Block executable-looking file names and upload content-types before they reach the destination.'),
    _field('file_security_executable_extensions', 'blocking', 'Executable extensions', 'urlpath_regex', 'text', help_text='Space or comma separated executable extensions without leading dots.', placeholder='exe dll msi com scr jar apk'),
    _field('file_security_block_archives', 'archives', 'Block archives', 'http_access deny urlpath_regex', 'checkbox', help_text='Block archive file names. Leave off for normal business use; enable for stricter environments.'),
    _field('file_security_archive_extensions', 'archives', 'Archive extensions', 'urlpath_regex', 'text', help_text='Space or comma separated archive extensions without leading dots.', placeholder='zip 7z rar tar gz bz2 xz iso'),
    _field('file_security_block_nested_archives', 'archives', 'Block nested archives', 'Policy metadata', 'checkbox', help_text='Records the stricter policy intent for downstream ICAP/content engines. Squid URL ACLs cannot prove nested archive structure by themselves.'),
    _field('file_security_max_download_size', 'size', 'Maximum download size', 'reply_body_max_size', 'text', help_text='Optional hard block for downloads above this size. 0 disables the size block.', placeholder='0'),
    _field('file_security_max_upload_size', 'size', 'Maximum upload size', 'request_body_max_size', 'text', help_text='Optional hard block for uploads above this size. 0 disables the size block.', placeholder='0'),
    _field('file_security_blocked_mime_types', 'types', 'Blocked upload content types', 'req_header Content-Type', 'text', help_text='Space or comma separated upload Content-Type hints to block when executable-content blocking is enabled.', placeholder='application/x-msdownload application/x-ms-installer'),
    _field('file_security_quarantine_metadata', 'diagnostics', 'Record quarantine metadata', 'Policy diagnostics', 'checkbox', help_text='Keep sample metadata in the managed policy block for diagnostics/quarantine workflows without storing file contents.'),
    ...
    _field('virus_scan_start_send_percent_after', 'streaming', 'Start early-send after', 'virus_scan.StartSendPercentDataAfter', 'text', help_text='Amount of response data c-icap should receive before early-send begins. Default 1K makes browser downloads show progress almost immediately.', placeholder='1K'),
...
CLAMAV_SECTIONS = (
    ClamavSectionSpec('virus_scan', 'File security policy', 'Schema-backed controls for file scanning, file blocking, c-icap virus_scan behavior, and Squid AV ICAP failure behavior.', 'Applies to the selected proxy by storing managed settings in the active config revision; the proxy materializes /etc/virus_scan.conf and the local ICAP include during sync.', (
        ClamavGroupSpec('preset', 'Preset', 'Start with a compact policy posture, then adjust specific controls only when needed.', ('file_security_preset',)),
        ...
        ClamavGroupSpec('coverage', 'Scan coverage', 'Uploads and downloads sent to c-icap virus_scan plus object classes handled by the scanner.', ('file_security_scan_downloads', 'file_security_scan_uploads', 'virus_scan_scan_file_types', 'virus_scan_max_object_size')),
        ClamavGroupSpec('blocking', 'File blocking', 'Fast Squid-side blocks for risky file names and executable-looking uploads.', ('file_security_block_risky_extensions', 'file_security_risky_extensions', 'file_security_block_executable_content', 'file_security_executable_extensions')),
        ClamavGroupSpec('archives', 'Archives', 'Archive controls with explicit metadata for nested archive handling.', ('file_security_block_archives', 'file_security_archive_extensions', 'file_security_block_nested_archives')),
        ClamavGroupSpec('size', 'Size thresholds', 'Optional hard caps for transfers that should be blocked before or during body handling.', ('file_security_max_download_size', 'file_security_max_upload_size')),
        ClamavGroupSpec('types', 'Upload content-type hints', 'Request Content-Type blocking where the existing Squid request path can enforce it. This is header-based, not deep file sniffing.', ('file_security_blocked_mime_types',)),
        ClamavGroupSpec('diagnostics', 'Diagnostics', 'Policy metadata that supports quarantine and audit trails without storing file contents.', ('file_security_quarantine_metadata',)),
...
def render_file_security_policy_config(options: Mapping[str, Any] | None = None) -> str:
    ...
    lines.append('acl file_security_upload_methods method POST PUT PATCH')
    lines.append('acl file_security_download_methods method GET HEAD')
    lines.append('acl file_security_range_request req_header Range .+')
    lines.append('acl file_security_partial_response http_status 206')

    if opts['file_security_block_risky_extensions'] and risky_exts:
        lines.append(f"acl file_security_risky_path urlpath_regex -i \\.{_render_regex_tokens(risky_exts)}(?:$|[?#])")
    if opts['file_security_block_archives'] and archive_exts:
        lines.append(f"acl file_security_archive_path urlpath_regex -i \\.{_render_regex_tokens(archive_exts)}(?:$|[?#])")
    if opts['file_security_block_executable_content'] and exec_exts:
        lines.append(f"acl file_security_executable_path urlpath_regex -i \\.{_render_regex_tokens(exec_exts)}(?:$|[?#])")
    if opts['file_security_block_executable_content'] and blocked_mimes:
        lines.append(f"acl file_security_executable_mime req_header Content-Type -i {_render_regex_tokens(blocked_mimes)}")
    if opts['file_security_block_nested_archives']:
        lines.append('# Nested archive blocking is recorded as policy metadata for downstream scanners.')
    if opts['file_security_quarantine_metadata']:
        lines.append('# Quarantine metadata is retained in the managed policy block for audit trails.')
    ...
    if opts['file_security_block_executable_content']:
        ...
        if blocked_mimes:
            lines.append('http_access deny file_security_executable_mime file_security_upload_methods')
...
def normalize_clamav_options(values: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        'file_security_scan_downloads': _clean_bool(source.get('file_security_scan_downloads'), bool(preset_defaults['file_security_scan_downloads'])),
        'file_security_scan_uploads': _clean_bool(source.get('file_security_scan_uploads'), bool(preset_defaults['file_security_scan_uploads'])),
        'file_security_block_risky_extensions': _clean_bool(source.get('file_security_block_risky_extensions'), bool(preset_defaults['file_security_block_risky_extensions'])),
        'file_security_risky_extensions': _clean_token_list(source.get('file_security_risky_extensions'), str(preset_defaults['file_security_risky_extensions'])),
        'file_security_block_archives': _clean_bool(source.get('file_security_block_archives'), bool(preset_defaults['file_security_block_archives'])),
        'file_security_archive_extensions': _clean_token_list(source.get('file_security_archive_extensions'), str(preset_defaults['file_security_archive_extensions'])),
        'file_security_block_nested_archives': _clean_bool(source.get('file_security_block_nested_archives'), bool(preset_defaults['file_security_block_nested_archives'])),
        'file_security_block_executable_content': _clean_bool(source.get('file_security_block_executable_content'), bool(preset_defaults['file_security_block_executable_content'])),
        'file_security_executable_extensions': _clean_token_list(source.get('file_security_executable_extensions'), str(preset_defaults['file_security_executable_extensions'])),
        'file_security_blocked_mime_types': ' '.join(_split_mime_tokens(source.get('file_security_blocked_mime_types') or preset_defaults['file_security_blocked_mime_types'])) or str(preset_defaults['file_security_blocked_mime_types']),
        'file_security_max_download_size': _clean_size(source.get('file_security_max_download_size'), str(preset_defaults['file_security_max_download_size'])),
        'file_security_max_upload_size': _clean_size(source.get('file_security_max_upload_size'), str(preset_defaults['file_security_max_upload_size'])),
        'file_security_quarantine_metadata': _clean_bool(source.get('file_security_quarantine_metadata'), bool(preset_defaults['file_security_quarantine_metadata'])),
...
def read_clamav_options_from_form(form: FormMap, current: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    requested_preset = _normalize_preset_name(form.get('file_security_preset', current_values.get('file_security_preset')))
    ...
    if preset_changed:
        preset_values = normalize_clamav_options({'file_security_preset': selected_preset})

...
_TLS_LIB_ERR_PATTERN = re.compile(r"\bTLS_LIB_ERR=([0-9A-F]+)\b", re.IGNORECASE)
_TLS_IO_ERR_PATTERN = re.compile(r"\bTLS_IO_ERR=(\d+)\b", re.IGNORECASE)
_TLS_ERROR_SIGNATURE_PATTERN = re.compile(
...
def _infer_ssl_category(category: str, *, reason: str, sample: str) -> str:
    ...
    if (
        "cannot accept a tls connection" in combined
        or "failure while accepting a tls connection" in combined
    ):
        return "TLS_CLIENT_ACCEPT"
    if "squid_tls_err_accept" in combined and (
        "tls_lib_err=a000119" in combined
        or "decryption failed or bad record mac" in combined
    ):
        return "TLS_CLIENT_ACCEPT"
    return key
...
def _display_reason(*, category: str, reason: str, sample: str) -> str:
    ...
    if category == "TLS_CLIENT_ACCEPT" and (
        "cannot accept a tls connection" in lowered
        or "failure while accepting a tls connection" in lowered
    ):
...
def _build_ssl_diagnostics(*, category: str, reason: str, sample: str) -> list[str]:
    ...
    if category == "TLS_CLIENT_ACCEPT":
        diagnostics.append(
            "This happened on the client -> proxy TLS leg, not the upstream site TLS session.",
        )
    ...
    if io_match:
        ...
        category == "TLS_CLIENT_ACCEPT"
        and "cannot accept a tls connection" in combined.lower()
    ):
        diagnostics.append(
            "Squid only logged the generic TLS accept failure line; there was no OpenSSL detail code in the latest sample.",
        )
    ...
    if remote or local:
        ...
        if conn_id:
            diagnostics.append(
                f"Latest connection context: {path_summary} ({conn_id}).",
            )
        else:
    ...
    if category == "TLS_CLIENT_ACCEPT" and not (remote or local) and not tx_match:
        diagnostics.append(
            "Squid did not emit any follow-up connection or master-transaction context for the latest sample, so cache.log alone cannot identify the client or app.",
        )
...
def present_icap_events(
    ...
    *,
    ...
    for row in rows[: max(1, limit)]:
        ...
        event["target_secondary"] = str(
            event.get("host") or event.get("sni") or "",
        ).strip()
...
def present_observability_summary(
    *,
    diagnostic_summary: dict[str, Any] | None = None,
    ssl_summary: dict[str, Any] | None = None,
) -> dict[str, int]:
...
def present_ssl_error_rows(rows: Sequence[Any]) -> dict[str, Any]:
    ...
    top_meta = (
        ssl_error_category_meta(top_category)
        if top_category
        else {
...
def present_ssl_exclusion_candidates(
    rows: Sequence[dict[str, Any]],
) -> list[dict[str, Any]]:
    presented: list[dict[str, Any]] = []
    for row in rows:
...
def append_query_to_local_return(return_to: str | None, **params: Any) -> str | None:
    ...
    return urlunsplit(
        ("", "", parsed.path, urlencode(items, doseq=True), parsed.fragment),
    )

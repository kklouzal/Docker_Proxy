...
def test_live_ui_pages_render_and_include_csrf_meta(
    admin_client: LiveStackClient, path: str, expected: str
) -> None:
...
def test_live_logged_in_layout_renders_shell_accessibility_hooks(
...
def test_live_login_page_uses_updated_auth_shell(
...
def test_live_admin_ui_pac_endpoints_are_removed(admin_client: LiveStackClient) -> None:
    for path in ("/proxy.pac", "/wpad.dat"):
        ...
        assert response.status == 404, (
            f"Expected admin UI to reject {path}, got HTTP {response.status}."
        )


def test_live_proxy_serves_pac_and_wpad(
    live_stack_ready: dict[str, dict[str, object]],
) -> None:
    _ = live_stack_ready
    client = LiveStackClient()

...
def test_sync_from_db_reconfigures_squid_after_adblock_artifact_change() -> None:
    ...
    runtime.sync_pac_state = lambda force=False: {"ok": Tr…3664 tokens truncated…          recorded.append(
...
def test_sync_adblock_state_reports_squid_regex_refresh_failure() -> None:
    ...
    class Artifacts:
        ...
        def get_active_artifact(
            ...
            msg = "current artifact should not be fetched when regex refresh fails"
            raise AssertionError(msg)
        ...
        def record_apply_result(self, proxy_id, revision_id, **kwargs):
            ...
            return SimpleNamespace(application_id=13)
    ...
    runtime.adblock_artifacts = Artifacts()
    runtime.adblock_store = Store()
    runtime.adblock_compiled_dir = "compiled"
    ...
    assert result["changed"] is False
    assert result["artifact_changed"] is False
    assert result["squid_regex_changed"] is False
    assert result["application_id"] == 13
    assert result["detail"] == "Failed to refresh Squid adblock regex ACL files."
    assert restarts == []
    assert recorded == [
        {
            "proxy_id": runtime.proxy_id,

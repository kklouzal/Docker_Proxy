...
class FakeConnection:
    def __init__(self, existing_tables: set[str], *, fail_first_table_exists: bool = False) -> None:
    ...
    def execute(self, sql: str, params: tuple[object, ...] | None = None) -> FakeResult:
        ...
        if "information_schema.tables" in sql:
            ...
            if self.fail_first_table_exists and self.table_exists_calls == 1:
                raise RuntimeError("InterfaceError: (0, '')")
            return FakeResult(1)
        return FakeResult(0)


def test_clear_observability_logs_truncates_all_known_log_tables_without_proxy_filter(monkeypatch) -> None:
    ...
    monkeypatch.setattr(maintenance, "table_exists", lambda _conn, table: table in existing)

    result = maintenance.clear_observability_logs(optimize=True)

    truncate_statements = [sql for sql in conn.statements if sql.strip().upper().startswith("TRUNCATE TABLE")]
    ...
    assert {row["table"] for row in result["tables"] if row["status"] == "cleared"} == existing
...
def test_clear_observability_logs_skips_tables_that_do_not_exist(monkeypatch) -> None:
    ...
    assert not any("ssl_errors" in sql and sql.strip().upper().startswith("TRUNCATE TABLE") for sql in conn.statements)


def test_clear_observability_logs_retries_stale_connection_on_table_probe(monkeypatch) -> None:
    existing = set(maintenance.OBSERVABILITY_LOG_TABLES)
    ...
    def fake_table_exists(fake_conn: FakeConnection, table: str) -> bool:
        fake_conn.execute("SELECT 1 FROM information_schema.tables WHERE table_name = %s", (table,))

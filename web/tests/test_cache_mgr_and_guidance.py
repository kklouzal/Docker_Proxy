...
def test_cache_mgr_default_metadata_and_rendered_config() -> None:
    ...
    assert (
        "cache_mgr proxy-admin@example.invalid"
        in controller.generate_config_from_template(options)
    )
...
def test_cache_mgr_form_override_renders_cache_mgr() -> None:
    options = build_template_options_from_form(
        {},
        {"cache_mgr_email": "helpdesk@example.test"},
        form_kind="http",
        max_workers=4,
    )
    controller = SquidController()
    controller.squid_conf_template_path = str(ROOT / "squid" / "squid.conf.template")
    assert (
        "cache_mgr helpdesk@example.test"
        in controller.generate_config_from_template(options)
    )
...
def test_branded_error_pages_have_user_guidance_before_request_details() -> None:
    ...
    for page in pages:
        ...
        assert text.index('<div class="user">') < text.index(
            'aria-label="Request details"'
        )
        assert "%U" in text
        assert "%T" in text
    dns = (ROOT / "squid" / "error_pages" / "en" / "ERR_DNS_FAIL").read_text(
        encoding="utf-8"
    )
